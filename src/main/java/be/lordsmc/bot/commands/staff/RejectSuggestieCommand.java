...
public class RejectSuggestieCommand extends Command {
    public RejectSuggestieCommand() {
        super("reject", Permission.ADMINISTRATOR, "Reject een suggestie");
    }
        ...
        if (msg.getMember().hasPermission(Permission.ADMINISTRATOR)) {
            if (args.length == 0) {
                ...
                EmbedBuilder embedBuilder = new EmbedBuilder(bericht.getEmbeds().get(0))
                        ...
                        .setFooter("Deze suggestie is afgekeurd!")
                        .setTimestamp(null);
                bericht.editMessage(embedBuilder.build()).queue();
                bericht.clearReactions().queue();

                msg.getChannel().sendMessage("Suggestie afgekeurd ❌").queue();
            }
        }
    }
}
